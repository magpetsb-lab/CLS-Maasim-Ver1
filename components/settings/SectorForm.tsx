...
interface SectorFormProps {
    ...
    onSubmit: (data: Omit<Sector, 'id'> | Sector) => void;
    onCancel: () => void;
}

const getInitialFormData = (): Omit<Sector, 'id'> => ({
    name: '',
});

const SectorForm: React.FC<SectorFormProps> = ({ initialData, onSubmit, onCancel }) => {
    ...
    return (
        <div className="bg-white p-6 sm:p-8 rounded-lg shadow-lg max-w-xl mx-auto border border-slate-200">
            <h2 className="text-2xl font-bold text-brand-primary mb-6">
                {initialData ? 'Edit Sector' : 'Add New Sector'}
            </h2>
            <form onSubmit={handleSubmit} className="space-y-6">
                <div>
                    <label htmlFor="name" className={labelClasses}>Sector Name</label>
                    <input
                        type="text"
                        id="name"
                        name="name"
