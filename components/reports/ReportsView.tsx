    ...
    const handleGeneratePDF = async (action: 'save' | 'preview') => {
        try {
            ...
            (doc as any).autoTable({ head: [tableColumn], body: tableRows, startY: startY, theme: 'grid', headStyles: { fillColor: [30, 58, 138] }, styles: { fontSize: 11 }, columnStyles: columnStyles, didDrawPage: function (data: any) {
                ...
                doc.text(`Total No. of Records: ${tableRows.length}`, doc.internal.pageSize.width - data.settings.margin.right, doc.internal.pageSize.height - 12, { align: 'right' });
                    ...
                    <div className="space-y-4">
                        ...
                        <div>
                            ...
                            <select value={selectedTerm} onChange={(e) => setSelectedTerm(e.target.value)} className={inputClasses}>
                                ...
                                {[...terms].sort((a, b) => parseInt(b.yearFrom) - parseInt(a.yearFrom)).map(term => <option key={term.id} value={`${term.yearFrom}-${term.yearTo}`}>{`${term.yearFrom.split('-')[0]}-${term.yearTo.split('-')[0]}`}</option>)}
                            </select>
                        </div>
                            ...
                            <table className="min-w-full border-collapse">
                                <thead>
                                    <tr className="bg-slate-100">
                                        ...
                                        ) : reportType === 'ElectiveOfficials' ? (
                                            <>
                                                <th className="border-b border-slate-200 px-4 py-2 text-left text-xs font-bold text-slate-700 uppercase">Official Name</th>
                                                <th className="border-b border-slate-200 px-4 py-2 text-left text-xs font-bold text-slate-700 uppercase">Position</th>
                                                <th className="border-b border-slate-200 px-4 py-2 text-left text-xs font-bold text-slate-700 uppercase">Rank</th>
                                                <th className="border-b border-slate-200 px-4 py-2 text-left text-xs font-bold text-slate-700 uppercase">Term</th>
                                            </>
                                        ) : reportType === 'StandingCommittees' ? (
                                            <>
                                                <th className="border-b border-slate-200 px-4 py-2 text-left text-xs font-bold text-slate-700 uppercase">Committee Name</th>
                                                <th className="border-b border-slate-200 px-4 py-2 text-left text-xs font-bold text-slate-700 uppercase">Chairman</th>
                                                <th className="border-b border-slate-200 px-4 py-2 text-left text-xs font-bold text-slate-700 uppercase">Vice-Chairman</th>
                                                <th className="border-b border-slate-200 px-4 py-2 text-left text-xs font-bold text-slate-700 uppercase">Member/s</th>
                                            </>
                                        ) : (
                                ...
                                <tbody>
                                    {reportType !== 'Attendance' && reportType !== 'CommitteeAttendance' && reportType !== 'ElectiveOfficials' && reportType !== 'StandingCommittees' && filteredData.slice(0, 15).map((item, idx) => (
                                    ...
                                    {reportType === 'Attendance' && attendanceData.slice(0, 15).map((stat, idx) => (
                                        <tr key={idx} className="hover:bg-slate-50 transition-colors">
                                    ...
                                    {reportType === 'CommitteeAttendance' && committeeAttendanceData.slice(0, 15).map((stat, idx) => (
                                        <tr key={idx} className="hover:bg-slate-50 transition-colors">
                                            <td className="border-b border-slate-100 px-4 py-2 text-sm text-slate-700">{stat.name}</td>
                                            <td className="border-b border-slate-100 px-4 py-2 text-center text-sm font-bold text-emerald-600">{stat.meetings}</td>
                                            <td className="border-b border-slate-100 px-4 py-2 text-center text-sm text-blue-600">{stat.hearings}</td>
                                            <td className="border-b border-slate-100 px-4 py-2 text-center text-sm font-black text-slate-900 bg-slate-50">{stat.total}</td>
                                        </tr>
                                    ))}
                                    {reportType === 'ElectiveOfficials' && electiveOfficialsData.slice(0, 15).map((off, idx) => (
                                        <tr key={idx} className="hover:bg-slate-50 transition-colors">
                                            <td className="border-b border-slate-100 px-4 py-2 text-sm text-slate-700">{off.name}</td>
                                            <td className="border-b border-slate-100 px-4 py-2 text-sm text-slate-700">{off.title}</td>
                                            <td className="border-b border-slate-100 px-4 py-2 text-sm text-slate-700">{off.rank}</td>
                                            <td className="border-b border-slate-100 px-4 py-2 text-sm text-slate-700">{formatTerm(off.term)}</td>
                                        </tr>
                                    ))}
                                    {reportType === 'StandingCommittees' && standingCommitteesData.slice(0, 15).map((com, idx) => (
                                        <tr key={idx} className="hover:bg-slate-50 transition-colors">
                                            <td className="border-b border-slate-100 px-4 py-2 text-sm text-slate-700">{com.committeeName}</td>
                                            <td className="border-b border-slate-100 px-4 py-2 text-sm text-slate-700">{com.chairman}</td>
                                            <td className="border-b border-slate-100 px-4 py-2 text-sm text-slate-700">{com.viceChairman}</td>
                                            <td className="border-b border-slate-100 px-4 py-2 text-sm text-slate-700 whitespace-normal break-words max-w-xs">{com.members}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
